...
void s_exit( void){printf("GoodBye\n"); exit(0);}/* Exit program */
...
struct { char *name; Func f; } op_tbl[] =
{
...
void parse_word( char word[]){
  int i, string_s; double x;

  for(i = 0; i < 13; ++i){
    if(strcmp(word, op_tbl[i].name) == 0){
      op_tbl[i].f(); return;
    }/*if*/
  }/*for*/

  string_s = sscanf(word, "%lf", &x);
  if(string_s != 1){ printf("Invalid Input\n"); return;}
  PUSH(x);
